...
export function buildLandingPage(entry: LandingCatalogEntry): LandingPageConfig {
  ...
  const introVariants = [
    ...
    `Windows screen recorders usually capture the whole desktop. For ${kw}, you want a deliberate ${aspectShort} viewport instead. ninesixteen.video records that region live — perfect for ${audience} who ship fast.`,
  ];
