...
impl VideoSettingsBuilder {
    ...
    /// Enables or disables hardware-accelerated encoding via Media Foundation.
    pub const fn hardware_acceleration(mut self, enabled: bool) -> Self {
        self.hardware_acceleration = enabled;
        self
    }

    fn build(self) -> Result<(VideoEncodingProperties, bool, bool), VideoEncoderError> {
        ...
        Ok((properties, self.disabled, self.hardware_acceleration))
    }
}
...
impl VideoEncoder {
    ...
    pub fn new<P: AsRef<Path>>(
        ...
        let (video_encoding_properties_cfg, is_video_disabled, hw_accel) = video_settings.build()?;
        ...
        media_transcoder.SetHardwareAccelerationEnabled(hw_accel)?;
    ...
    /// Sends a pre-scaled Direct3D surface (GPU path). `timestamp_ticks` uses
    /// the same 100 ns units as [`Self::send_frame_buffer`].
    #[inline]
    pub fn send_surface(
        &mut self,
        surface: SendDirectX<IDirect3DSurface>,
        ...
        let timestamp = match self.first_timestamp {
            Some(t0) => TimeSpan { Duration: timestamp_ticks - t0.Duration },
            None => {
                self.first_timestamp = Some(TimeSpan { Duration: timestamp_ticks });
                TimeSpan { Duration: 0 }
            }
        };
